...
use denali_core::pipeline::{Builder, SendMode};
use std::collections::HashMap;
use tokio::signal;
use tokio::sync::mpsc;

use denali_storage::trades::write_trades_to_parquet;
use denali_storage::candles::write_candles_to_parquet;
use denali_storage::l2::write_l2_to_parquet;
use denali_storage::user_fills::write_user_fills_to_parquet;

use denali_trade_engine::trades::{clean_trade, should_persist_trade};
use denali_trade_engine::candles::{clean_candle, should_persist_candle};
use denali_trade_engine::l2::{clean_l2book, should_persist_l2};
use denali_trade_engine::user_fills::{clean_fill, should_persist_fill};

#[tokio::main]
async fn main() -> Result<()> {
    let coins: Vec<String> = vec!["BTC".into(), "ETH".into(), "SOL".into()];
    let interval = "1m".to_string();
    let user_address = "0x0c3391b8cC5CA37b4a8f1E01432dAF1a933394cF".to_string();
    ...
    // Trades
    let mut trades_pipelines: HashMap<String, denali_core::pipeline::Pipeline<Trade>> = HashMap::new();
    ...
    // Candles
    let mut candles_pipelines: HashMap<String, denali_core::pipeline::Pipeline<Candle>> = HashMap::new();
    ...
    // L2
    let mut l2_pipelines: HashMap<String, denali_core::pipeline::Pipeline<L2Book>> = HashMap::new();
    ...
    loop {
        tokio::select! {
            Some(msg) = ws.next() => {
                i += 1;
                match msg {
                    hypersdk::hypercore::ws::Event::Message(Incoming::Trades(trades)) => {
                        if !trades.is_empty() {
